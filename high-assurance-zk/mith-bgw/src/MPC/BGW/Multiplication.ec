...
theory BGWMultiplication.
  ...
  clone import MultiplicationGate with
    ...
    op eval = eval
  proof MultiplicationFunctionality.GateFunctionality.n_pos, 
        MultiplicationFunctionality.GateFunctionality.t_pos,
        MultiplicationFunctionality.GateFunctionality.pid_set_size,
        MultiplicationFunctionality.GateFunctionality.pid_set_uniq,
        Gate.GateFunctionality.n_pos,
        Gate.GateFunctionality.t_pos,
        MultiplicationFunctionality.SecretSharingScheme.n_pos,
        MultiplicationFunctionality.SecretSharingScheme.t_pos,
        MultiplicationFunctionality.SecretSharingScheme.pid_set_size,
        MultiplicationFunctionality.SecretSharingScheme.pid_set_uniq,
        MultiplicationFunctionality.SecretSharingScheme.correct,
        MultiplicationFunctionality.SecretSharingScheme.size_share,
        MultiplicationFunctionality.SecretSharingScheme.unzip1_share,
        MultiplicationFunctionality.SecretSharingScheme.public_encoding_correct,
        MultiplicationFunctionality.SecretSharingScheme.size_public_encoding,
        MultiplicationFunctionality.SecretSharingScheme.unzip1_public_encoding,
        MultiplicationFunctionality.SecretSharingScheme.public_encoding_inj,
        MultiplicationFunctionality.SecretSharingScheme.public_encoding_reconstruct,
        MultiplicationFunctionality.SecretSharingScheme.public_encoding_rand,
...
rewrite update_expo => /#.
    ...
    exists (interpolate_poly (map (fun pid => (pid, interpolate fzero (map (fun pid' => (pid', eval pid (update (oget (assoc rs pid')) 0 (fmul (eval pid' (update r 0 s)) (eval pid' (update r' 0 s')))))) pid_set))) pid_set)); split. 
      ...
      have ->: (map (fun (pid : coeficient) => (pid, interpolate fzero (map (fun (pid' : pid_t) => (pid', eval pid (update (oget (assoc rs pid')) 0 (fmul (eval pid' (update r 0 s)) (eval pid' (update r' 0 s')))))) pid_set))) pid_set) = (map (fun (pid : coeficient) => (pid, eval fzero (interpolate_poly (map (fun (pid' : pid_t) => (pid', eval pid (update (oget (assoc rs pid')) 0 (fmul (eval pid' (update r 0 s)) (eval pid' (update r' 0 s')))))) pid_set)))) pid_set)
        by rewrite -eq_in_map; progress; rewrite interpolate_poly_eval; first rewrite -map_comp /(\o) /= map_id pid_set_uniq.
      by done. 
    ...
    have ->: (map (fun (pid0 : coeficient) => (pid0, interpolate fzero (map (fun (pid' : pid_t) => (pid', eval pid0 (update (oget (assoc rs pid')) 0 (fmul (eval pid' (update r 0 s)) (eval pid' (update r' 0 s')))))) pid_set))) pid_set) = (map (fun (pid0 : coeficient) => (pid0, eval fzero (interpolate_poly (map (fun (pid' : pid_t) => (pid', eval pid0 (update (oget (assoc rs pid')) 0 (fmul (eval pid' (update r 0 s)) (eval pid' (update r' 0 s')))))) pid_set)))) pid_set).
      rewrite -eq_in_map; progress.
      by rewrite interpolate_poly_eval; first by rewrite -map_comp /(\o) map_id pid_set_uniq.
    rewrite eval_update.
      ...
      rewrite get_monomial0 //.
        rewrite expo_diff_count //. 
          by rewrite H14; smt (t_pos).
          by rewrite H14; progress; rewrite H15. 
    rewrite /meval /= !pow0 -(interpolate_poly_eval _ fzero); first by rewrite -map_comp /(\o) map_id pid_set_uniq.
    ...
    rewrite -interpolate_poly_eval; first by
      rewrite -map_comp /(\o) map_id pid_set_uniq.
    ringeq.
qed.
