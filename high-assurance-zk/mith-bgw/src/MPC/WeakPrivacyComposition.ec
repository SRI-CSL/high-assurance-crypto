...
theory WeakPrivacyComposition.
  ...
  clone import Gate as SG with
    ...
    op GateFunctionality.f (x : inputs_t) =
      let ys = map (fun pid => (pid, (sinput pid x))) pid_set in
    ...
    op GateFunctionality.valid_inputs xs =
      ...
      (exists r s, valid_secret s /\ SecretSharingScheme.valid_rand r s /\ share r s = sec).

  lemma good_refresh s : 
    (forall xs xs', 
      reconstruct (map (fun pid => (pid, (SG.GateFunctionality.sinput pid xs))) SecretSharingScheme.pid_set) = s => 
      reconstruct (map (fun pid => (pid, (SG.GateFunctionality.sinput pid xs'))) SecretSharingScheme.pid_set) = s =>
...
exists (reconstruct (map (fun (pid : GateFunctionality.pid_t) => (pid, (GateFunctionality.sinput pid xs))) SecretSharingScheme.pid_set)).
rewrite allP /f /=.
move => x.
rewrite mapP.
done.
qed.
  ...
  axiom strong_valid_inputs_from_weak topo gg rs xs : 
    ...
    ProtocolFunctionality.valid_inputs (topo,gg) xs =>
    let ys = (eval_circuit gg rs xs).`2 in
    SG.GateFunctionality.valid_inputs (map (fun pid => (pid, ((), (oget (assoc ys pid))))) ProtocolFunctionality.pid_set).

  type msgs_t = ArithmeticProtocol.msgs_t * SG.msgs_t.
  type in_messages_t = gate_local_traces_t * SG.in_messages_t.
  type out_messages_t = ArithmeticProtocol.out_messages_t * SG.out_messages_t.
  ...
  op get_messages_to pid om =
    let (omwp, omsp) = om in (ArithmeticProtocol.get_messages_to pid omwp, SG.get_messages_to pid omsp).
  ...
  clone import Protocol as PP with
    type ProtocolFunctionality.circuit_t = ArithP.ProtocolFunctionality.circuit_t,
    op ProtocolFunctionality.valid_circuit = ArithP.ProtocolFunctionality.valid_circuit,
...
(*      let ys = map (fun pid => (pid, y)) pid_set in ys.
    ...
    op protocol (c : ProtocolFunctionality.circuit_t) (rs : rands_t) (xs : ProtocolFunctionality.inputs_t) = 
      let (topo, gg) = c in
      ...
      let xsp = map (fun pid => (pid, ((), (oget (assoc ys pid))))) ProtocolFunctionality.pid_set in
    ...
    op local_output c pid v = 
      ...
      let zi = SG.local_output pid (((),(yi)), risp, (ys, imssp)) in
      zi

    (*op messages_consistency c vi vj i j =
      let (xi, ri, ti) = vi in
      let (xj, rj, tj) = vj in*)
    ...
    proof ProtocolFunctionality.n_pos, ProtocolFunctionality.t_pos,
          ...
          correct, local_output_correct, correct_domain.
    ...
    realize correct.
      rewrite /valid_inputs /valid_input /valid_rands /valid_rand /protocol /f /=
              /pinput /sinput /input /output /trace /= /ProtocolFunctionality.valid_circuit 
              /ArithP.ProtocolFunctionality.valid_circuit => c rs xs => /> H H0; move : H.
      ...
      progress; rewrite -H4 /=.
      have : exists (trsp, zs), (trsp, zs) = gate (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)
        by exists (gate (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)).`1
                  (gate (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)).`2 => /#.
      progress; rewrite -H5 /=.
...
move : (correct_eval_gates (np,ns,m,q) gg (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`1)) ProtocolFunctionality.pid_set) xs ((oget (assoc xs (head witness SecretSharingScheme.pid_set))).`1, map
     (fun (sec : (SecretSharingScheme.pid_t * PrimeField.t) list) =>
        sumt (unzip2 sec))
     (map
        (fun (k : int) =>
           map
             (fun (pid0 : SecretSharingScheme.pid_t) =>
                (pid0, nth witness (oget (assoc xs pid0)).`2 k))
...
move : (H9 k); rewrite H12 H13 /=.
progress.
move : H15; rewrite allP; progress.
...
rewrite -H8.
      ...
      move : (SG.correct (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)).
      have ->: (GateFunctionality.valid_inputs (map (fun (pid0 : ProtocolFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys pid0))))) ProtocolFunctionality.pid_set)).
        ...
        have ->: ArithmeticProtocolFunctionality.ProtocolFunctionality.valid_inputs ((np, ns, m, q), gg) xs by smt().
        simplify.
        ...
        (map
           (fun (pid1 : GateFunctionality.pid_t) =>
              (pid1,
               (oget
                  (assoc
                     (map (fun (pid2 : ProtocolFunctionality.pid_t) => (pid2, (tt, (oget (assoc ys pid2)))))
                        pid_set) pid1)).`2)) GateFunctionality.pid_set))) GateFunctionality.pid_set =
map
  (fun (pid0 : SecretSharingScheme.pid_t) =>
     (pid0,
...
have ->: (map
     (fun (pid1 : GateFunctionality.pid_t) =>
        (pid1,
         (oget
            (assoc
               (map (fun (pid2 : ProtocolFunctionality.pid_t) => (pid2, (tt, (oget (assoc ys pid2))))) pid_set)
               pid1)).`2)) GateFunctionality.pid_set) = 
(map
     (fun (pid1 : GateFunctionality.pid_t) =>
...
have ->: zs = (gate (map (fun (pid0 : ProtocolFunctionality.pid_t) => (pid0, (rand pid0 rs).`2)) pid_set)
        (map (fun (pid0 : ProtocolFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys pid0))))) pid_set)).`2 by smt().
rewrite H12.
rewrite !map_assoc //=.
smt().
    qed.
    realize local_output_correct.
      ...
      rewrite -H1 /=.
      have : exists (trsp, zs), (trsp, zs) = gate (map (fun (pid0 : ProtocolFunctionality.pid_t) => (pid0, (rand pid0 rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys' pid0))))) ProtocolFunctionality.pid_set) 
        by exists (gate (map (fun (pid0 : ProtocolFunctionality.pid_t) => (pid0, (rand pid0 rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys' pid0))))) ProtocolFunctionality.pid_set)).`1 (gate (map (fun (pid0 : ProtocolFunctionality.pid_t) => (pid0, (rand pid0 rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys' pid0))))) ProtocolFunctionality.pid_set)).`2 => /#.
      progress; rewrite -H2 /=.
      rewrite !map_assoc //=.
      move : (SG.local_output_correct pid (map (fun (pid : pid_t) => (pid, (tt, (oget (assoc ys' pid))))) pid_set) (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set)).
...
qed.
    realize correct_domain.
      ...
      have : exists (trsp, zs), (trsp, zs) = gate (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)
        by exists (gate (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)).`1
                  (gate (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)).`2 => /#.
      move => H4; elim H4 => trsp zs H4; rewrite -H4 /=.
      have ->: zs = (gate (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : pid_t) => (pid, (tt, (oget (assoc ys pid))))) ProtocolFunctionality.pid_set)).`2 by smt().
      ...
      move : (SG.correct_domain (map (fun (pid : ProtocolFunctionality.pid_t) => (pid, (rand pid rs).`2)) ProtocolFunctionality.pid_set) (map (fun (pid : pid_t) => (pid, ((), (oget (assoc ys pid)))))
           ...
           (pid,
            (tt,
             (oget
               (assoc
                  (eval_gates gg
                     (map (fun (pid0 : ProtocolFunctionality.pid_t) => (pid0, (rand pid0 rs).`1)) pid_set) xs).`2 pid)))))
        pid_set) = 
(map
        (fun (pid : pid_t) =>
           (pid,
...
smt().
  ...
  clone import GatePrivacy.Privacy as SP_S with 
    theory Gate <- SG, 
    type aux_t = WP_S.aux_t * ProtocolFunctionality.circuit_t, 
  ...
  section Proof.
    ...
    module RealEvaluator1 = {
      proc eval(c : ProtocolFunctionality.circuit_t, xs : PP.ProtocolFunctionality.inputs_t, cr : PP.ProtocolFunctionality.pid_t list, rs : rands_t) : views_t = {
        var vsc, rwp, trwp, ys;
        ...
        xsp <- map (fun pid => (pid, ((), (oget (assoc ys pid))))) pid_set;
    ...
    module D_SP (D : Privacy.Distinguisher_t) (R : WP_S.Rand_t) = {
      ...
      proc choose(x : env_input_t, aux : SP_S.aux_t) : SG.GateFunctionality.inputs_t * SG.GateFunctionality.pid_t list = {
        ...
        if (ProtocolFunctionality.valid_inputs c xs /\ ArithP.valid_rands c rwp /\ WP_S.valid_corrupted_set cr) {
          (trwp, ys) <- eval_gates (snd c) rwp xs;
          xsp <- map (fun pid => (pid, ((), (oget (assoc ys pid))))) ProtocolFunctionality.pid_set;
        }
    ...
    module D_WP (D : Privacy.Distinguisher_t) (R : SP_S.Rand_t) = {
      ...
      var cr : SG.GateFunctionality.pid_t list
      var aux : WP_S.aux_t
      var c : ProtocolFunctionality.circuit_t

      proc choose(c_: ProtocolFunctionality.circuit_t, x : env_input_t, aux_ : WP_S.aux_t) : ProtocolFunctionality.inputs_t * ProtocolFunctionality.pid_t list = {
      ...
      proc guess(ys : ArithP.poutputs_t, vsc : weak_views_t) : bool = {
        ...
        if (valid_rands rsp) {
          ...
          xspc <- map (fun pid => (pid, ((), (local_output_gates pid (oget (assoc vsc pid)).`1 (oget (assoc vsc pid)).`2 (oget (assoc vsc pid)).`3)))) cr;
          vsp <- SP_S.simulator rsp xspc cr ysfc;
    ...
    module Simulator = {
      proc simm(c : ProtocolFunctionality.circuit_t, xs : PP.ProtocolFunctionality.inputs_t, rs : rands_t, cr : PP.ProtocolFunctionality.pid_t list, zs : PP.ProtocolFunctionality.outputs_t) : views_t = {
        ...
        rsp <- map (fun pid => (pid, snd (rand pid rs))) ProtocolFunctionality.pid_set;
          xspc <- map (fun pid => (pid, ((), (local_output_gates pid (oget (assoc vswp pid)).`1 (oget (assoc vswp pid)).`2 (oget (assoc vswp pid)).`3)))) cr;
    ...
    module R_PP (R_WP : WP_S.Rand_t) (R_SP : SP_S.Rand_t) : Privacy.Rand_t = {
      proc gen(c : ProtocolFunctionality.circuit_t, cr : ProtocolFunctionality.pid_t list, aux : aux_t) : rands_t = {
        var rwp, rsp, rs;
...
map
     ...
     (map
        (fun (k : int) =>
           map
             (fun (pid0 : SecretSharingScheme.pid_t) =>
                (pid0, nth witness (oget (assoc D_SP.xs{2} pid0)).`2 k))
             SecretSharingScheme.pid_set) (range 0 (np + ns))))).
...
move : (H11 k); rewrite H14 H15 /=.
progress.
move : H17; rewrite allP; progress.
...
have ->: map
  (fun (pid : GateFunctionality.pid_t) =>
     (pid,
      (oget
         (assoc
            (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys{2} pid0))))) ProtocolFunctionality.pid_set)
            pid)).`2)) GateFunctionality.pid_set = 
map
  (fun (pid : GateFunctionality.pid_t) =>
...
smt().
      ...
      have : exists (trsp, zs), (trsp, zs) = gate (map (fun (pid0 : pid_t) => (pid0, rand pid0 r{2})) ProtocolFunctionality.pid_set) (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys0 pid0))))) ProtocolFunctionality.pid_set)
        by exists (gate (map (fun (pid0 : pid_t) => (pid0, rand pid0 r{2})) ProtocolFunctionality.pid_set) (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys0 pid0))))) ProtocolFunctionality.pid_set)).`1
                  (gate (map (fun (pid0 : pid_t) => (pid0, rand pid0 r{2})) ProtocolFunctionality.pid_set) (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys0 pid0))))) ProtocolFunctionality.pid_set)).`2 => /#.
      progress; rewrite -H8 /=.
...
have ->: gate (map (fun (pid0 : pid_t) => (pid0, rand pid0 r{2})) ProtocolFunctionality.pid_set)
  (map (fun (pid0 : pid_t) => (pid0, (tt, (oget (assoc ys0 pid0)))))
...
rewrite !H10.
      ...
      have ->: (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, oget (assoc (map (fun (pid1 : pid_t) => (pid1, (tt, (oget (assoc ys1 pid1))))) ProtocolFunctionality.pid_set) pid0))) D_SP.cr{2}) = (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys1 pid0))))) D_SP.cr{2})
      ...
      have ->: (map (fun (pid2 : GateFunctionality.pid_t) =>
                                       (pid2,
                                        (oget
                                           (assoc
                                              (map
                                                 (fun (pid3 : pid_t) => (pid3, (tt, (oget (assoc ys1 pid3)))))
                                                 pid_set) pid2)).`2)) GateFunctionality.pid_set) = (map
...
rewrite !map_assoc //=.
rewrite /pinput /input /=.
move : (H16 k); rewrite H19 H20 /=.
progress.
move : H22; rewrite allP; progress.
...
have ->: (map
             (fun (pid0 : GateFunctionality.pid_t) =>
                (pid0,
                 oget
                   (assoc
                      (map (fun (pid1 : pid_t) => (pid1, (tt, (oget (assoc ys1 pid1))))) ProtocolFunctionality.pid_set)
                      pid0))) D_SP.cr{2}) =
(map (fun (pid0 : GateFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys1 pid0))))) D_SP.cr{2}).
rewrite -eq_in_map; progress.
                               ...
                               (map
                                  (fun (pid2 : GateFunctionality.pid_t) =>
                                     (pid2,
                                      (oget
                                         (assoc
                                            (map (fun (pid3 : pid_t) => (pid3, (tt, (oget (assoc ys1 pid3)))))
                                               ProtocolFunctionality.pid_set) pid2)).`2)) GateFunctionality.pid_set)))
...
smt().
      ...
      have : exists (xi, ri, tri), (xi, ri, tri) = oget (assoc (simulator r{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys1 pid0))))) D_SP.cr{2}) D_SP.cr{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, reconstruct ys{2})) D_SP.cr{2})) pid)
        by exists (oget (assoc (simulator r{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys1 pid0))))) D_SP.cr{2}) D_SP.cr{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, reconstruct ys{2})) D_SP.cr{2})) pid)).`1
                  (oget (assoc (simulator r{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys1 pid0))))) D_SP.cr{2}) D_SP.cr{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, reconstruct ys{2})) D_SP.cr{2})) pid)).`2
                  (oget (assoc (simulator r{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, (tt, (oget (assoc ys1 pid0))))) D_SP.cr{2}) D_SP.cr{2} (map (fun (pid0 : GateFunctionality.pid_t) => (pid0, reconstruct ys{2})) D_SP.cr{2})) pid)).`3 => /#.
                          ...
                          (assoc
                             (eval_gates gg
                                (map (fun (pid2 : pid_t) => (pid2, (rand pid2 r{2})%ArithP))
                                   ProtocolFunctionality.pid_set) D_WP.xs{2}).`2 pid1))))) pid_set) pid0)).`2))
   ...
   map
     ...
     (map
        (fun (k : int) =>
           map
             (fun (pid0 : SecretSharingScheme.pid_t) =>
                (pid0, nth witness (oget (assoc D_WP.xs{2} pid0)).`2 k))
             SecretSharingScheme.pid_set) (range 0 (np + ns))))).
have ->: (ArithmeticCircuit.Circuit.valid_topology (np,ns,m,q)) by smt().
have ->: (ArithmeticCircuit.valid_gates ((np,ns,m,q), gg)) by smt().
have ->: all
