...
type 'a list =
  ...
  type witness_t = (Z.t) list

  type instance_t = (Z.t) list
  ...
  type statement_t = (topology_t * gates_t) * ((Z.t) list)
  
  let rec nth : type a. a -> (a list) -> (Z.t) ->  a =
    fun x xs n -> match xs with
      | Nil -> x
      | Cons (y,
        ys) ->
        if logical_equality n Z.zero then y else nth x ys (Z.sub n Z.one)
  
  let rec eval_gates (gg: gates_t) (xp: (Z.t) list) (xs: (Z.t) list) : 
    Z.t =
    match gg with
  ...
  let eval_circuit (c: topology_t * gates_t) (xp: (Z.t) list) (xs: (Z.t) list) :
    Z.t = let (topo, gg) = c in eval_gates gg xp xs
  
  let relation (w: (Z.t) list) (x: (topology_t * gates_t) * ((Z.t) list)) :
    bool =
    let (c,
    inst) =
    x in
  ...
  let relation1 (w: (Z.t) list) (x: (topology_t * gates_t) * ((Z.t) list)) :
    bool = relation w x
  
  type prover_input_t = ((Z.t) list) * ((topology_t * gates_t) * ((Z.t) list))
  
  type verifier_input_t = (topology_t * gates_t) * ((Z.t) list)
  ...
  type prover_rand_t = ui_t list
  
  let boolean_and (b1: bool) (b'1: bool) : bool = b1 && b'1
  
  let rec valid_rand_gates (r: ui_t list) (gg1: gates_t) : bool =
    match gg1 with
    | PInput wid -> true
    | SInput wid -> true
  ...
  let valid_rand_gates1 (r: ui_t list) (gg1: gates_t) : bool =
    valid_rand_gates r gg1
  
  let boolean_not (b1: bool) : bool = not b1
  
  let rec size : type a. (a list) ->  (Z.t) =
    fun xs -> match xs with
      | Nil -> Z.zero
      | Cons (y, ys) -> (let result = size ys in Z.add Z.one result)
  
  let valid_rand_prover (r: ui_t list)
                        (x:
                        ((Z.t) list) * ((topology_t * gates_t) * ((Z.t) list))) :
    bool =
    let (w,
    st) =
    x in
  ...
  let valid_rand_prover1 (r: ui_t list)
                         (x:
                         ((Z.t) list) * ((topology_t * gates_t) * ((Z.t) list)))
  ...
  let rec gen_z (u: ui_t list) (gg2: gates_t) (xp: (Z.t) list) (xs: (Z.t) list) :
    z_t =
    match gg2 with
    | PInput wid ->
  ...
  let get_a (u: ui_t list) (gg2: gates_t) : Z.t =
    let gid = get_gid gg2 in let ui = nth def_ui u gid in ui.a
  
  let get_a1 (u: ui_t list) (gg2: gates_t) : Z.t = get_a u gg2
  ...
  let commit (r: ui_t list)
             (x: ((Z.t) list) * ((topology_t * gates_t) * ((Z.t) list))) :
    z_t * ((Z.t)) =
    let (w1,
    st1) =
    x in
  ...
  let prove (r: verifier_rand_t) (x: (topology_t * gates_t) * ((Z.t) list))
            (c6: z_t * ((Z.t))) : bool =
    let (z1,
    z') =
    c6 in
  ...
  let protocol (r: (ui_t list) * verifier_rand_t)
               (x:
               (((Z.t) list) * ((topology_t * gates_t) * ((Z.t) list))) *
               ((topology_t * gates_t) * ((Z.t) list))) :
    (z_t * ((Z.t))) * (unit * (bool)) =
    let (r_p,
    r_v) =
    r in
