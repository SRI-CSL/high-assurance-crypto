...
let dummy_rand prime = { a = Z.div prime (ofint 143) ;
...
let get_prover_rand (topo : topology_t) (prime : Z.t) : prover_rand_t =
  ...
  for i = 0 to (Z.to_int (Z.add (Z.add total_rand (Z.of_int 2)) Z.one)) - 1 do
    let u = real_rand_prover prime in
    rp := infix_plpl !rp (Cons(u, Nil)) ;
  done ;
...
let rec pad_witness' l i ws =
  ...
  else pad_witness' l (Z.add i Z.one) (infix_plpl (Cons (Z.zero, Nil)) ws)
...
let get_verifier_rand (topo : topology_t) (prime : Z.t) (rp : prover_rand_t) : verifier_rand_t =
  ...
  for i = 0 to Z.to_int (Z.add (Z.add total_rand (Z.of_int 2)) Z.one) - 1 do
    let yi = dummy_rand_verifier alpha (nth witness rp (Z.of_int i)) in
    rv := infix_plpl !rv (Cons(yi, Nil)) ;
  done ;
