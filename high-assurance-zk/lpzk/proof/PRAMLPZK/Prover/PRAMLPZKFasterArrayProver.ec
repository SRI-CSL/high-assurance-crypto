...
from LPZK require import LPZK LPZKFasterArray.
from Utilities require import ListExt.

from RAMProver require import RAMLPZKFasterArrayProver.

import LPZK.
import ArithmeticCircuit.
import Circuit.
import LPZKFasterArray.
...
theory PRAMProver.
  ...
  proof.
    ...
    have ->: (fun (st : int * z_t) (c : processorId_t * state_t) =>
           ...
           let (_, com) = c in
           (i + 1,
            AdditionZ i {| m = fzero;  |} acc
              (oget com.`zo))) = 
            (fun (st : int * z_t) (c : processorId_t * state_t) =>
           (st.`1 + 1,
            AdditionZ st.`1 {| m = fzero;  |} st.`2
              (oget c.`2.`zo))) by smt().
  ...
  axiom foldl_assumption (topo : topology_t) (cs : circuit_t list) (ys : output_wires_t)
    ...
    (foldl (fun (z : int * z_t) (x : processorId_t) => (z.`1 + 1, AdditionZ z.`1 {| m = fzero;  |} z.`2 (gen_z (oget (odflt empty_state (assoc pst x)).`r) (add_final_mul (nth witness cs x)).`gates (oget (odflt empty_state (assoc pst x)).`inst) (oget (odflt empty_state (assoc pst x)).`w)))) (topo.`ngates - topo.`noutputs, gen_z (oget (odflt empty_state (assoc pst 0)).`r) (add_final_mul (nth witness cs 0)).`gates (oget (odflt empty_state (assoc pst 0)).`inst) (oget (odflt empty_state (assoc pst 0)).`w)) (iota_ 0 nprocesses)).`2 =
  ...
  proof.
    ...
    simplify.
    have ->: Some (foldl (fun (z : int * z_t) (x : processorId_t) => (z.`1 + 1, AdditionZ z.`1 {| m = fzero;  |} z.`2 (gen_z (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`r) (add_final_mul (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`circ)).`gates (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`inst) (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`w)))) (topo.`ngates - topo.`noutputs, gen_z (oget (odflt empty_state (assoc pst 0)).`r) (add_final_mul (oget (init_state (nth witness cs 0)).`circ)).`gates (oget (odflt empty_state (assoc pst 0)).`inst) (oget (odflt empty_state (assoc pst 0)).`w)) (iota_ 0 nprocesses)).`2 = 
             Some (foldl (fun (z : int * z_t) (x : processorId_t) => (z.`1 + 1, AdditionZ z.`1 {| m = fzero;  |} z.`2 (gen_z (oget (odflt empty_state (assoc pst x)).`r) (add_final_mul (oget (init_state (nth witness cs x)).`circ)).`gates (oget (odflt empty_state (assoc pst x)).`inst) (oget (odflt empty_state (assoc pst x)).`w)))) (topo.`ngates - topo.`noutputs, gen_z (oget (odflt empty_state (assoc pst 0)).`r) (add_final_mul (oget (init_state (nth witness cs 0)).`circ)).`gates (oget (odflt empty_state (assoc pst 0)).`inst) (oget (odflt empty_state (assoc pst 0)).`w)) (iota_ 0 nprocesses)).`2.
      rewrite (eq_in_foldl (fun (z : int * z_t) (x : processorId_t) => (z.`1 + 1, AdditionZ z.`1 {| m = fzero;  |} z.`2 (gen_z (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`r) (add_final_mul (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`circ)).`gates (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`inst) (oget (set_input (init_state (nth witness cs x)) (oget (get_input (odflt empty_state (assoc pst x))))).`w)))) (fun (z : int * z_t) (x : processorId_t) => (z.`1 + 1, AdditionZ z.`1 {| m = fzero;  |} z.`2 (gen_z (oget (odflt empty_state (assoc pst x)).`r) (add_final_mul (oget (init_state (nth witness cs x)).`circ)).`gates (oget (odflt empty_state (assoc pst x)).`inst) (oget (odflt empty_state (assoc pst x)).`w)))) (topo.`ngates - topo.`noutputs, gen_z (oget (odflt empty_state (assoc pst 0)).`r) (add_final_mul (oget (init_state (nth witness cs 0)).`circ)).`gates (oget (odflt empty_state (assoc pst 0)).`inst) (oget (odflt empty_state (assoc pst 0)).`w)) (topo.`ngates - topo.`noutputs, gen_z (oget (odflt empty_state (assoc pst 0)).`r) (add_final_mul (oget (init_state (nth witness cs 0)).`circ)).`gates (oget (odflt empty_state (assoc pst 0)).`inst) (oget (odflt empty_state (assoc pst 0)).`w)) (iota_ 0 nprocesses) (iota_ 0 nprocesses)) //=.
        progress.
  ...
  clone import MapReduce with
    ...
    type FunctionalRAM.state_t = RAMLPZKFasterArrayProver.RAMProver.state_t,
    op FunctionalRAM.empty_state = RAMLPZKFasterArrayProver.RAMProver.empty_state,
    op FunctionalRAM.init_state = RAMLPZKFasterArrayProver.RAMProver.init_state,
    op FunctionalRAM.set_input = RAMLPZKFasterArrayProver.RAMProver.set_input,
    op FunctionalRAM.step = RAMLPZKFasterArrayProver.RAMProver.step,
    op FunctionalRAM.get_input = RAMLPZKFasterArrayProver.RAMProver.get_input,
    op FunctionalRAM.get_output = RAMLPZKFasterArrayProver.RAMProver.get_output,
    op FunctionalRAM.get_circuit = RAMLPZKFasterArrayProver.RAMProver.get_circuit,
    op FunctionalRAM.has_input = RAMLPZKFasterArrayProver.RAMProver.has_input,
    op FunctionalRAM.has_output = RAMLPZKFasterArrayProver.RAMProver.has_output,
    op FunctionalRAM.has_circuit = RAMLPZKFasterArrayProver.RAMProver.has_circuit,
