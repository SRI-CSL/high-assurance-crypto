(**
  Formalization of the array-based LPZK prover on top of the RAM model. Briefly, this file
...
theory RAMProver.
  ...
  clone import Language with
    type L = circuit_t,
    type public_t = LPZKFasterArray.instance_t,
    type secret_t = LPZKFasterArray.witness_t * LPZKFasterArray.prover_rand_t,
    type input_t = secret_t * public_t,
    type output_t = LPZKFasterArray.commitment_t.
  ...
  op step (st : state_t) : state_t * execution_info_t option = 
    if has_input st then
      ...
      let zc = LPZKFasterArray.gen_z (oget st.`r) c'.`gates (oget st.`inst) (oget st.`w) in
  ...
  op get_output (st : state_t) : output_t option = 
    if st.`zo <> None then
      let z = oget st.`zo in
      let z' = (LPZKFasterArray.get_a (oget st.`r) (oget st.`circ).`gates) in
      Some (z, z')
    else None.
  ...
  (** Concrete array-based prover in the RAM model *)
