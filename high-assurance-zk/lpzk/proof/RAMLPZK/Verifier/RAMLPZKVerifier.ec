(**
  Formalization of the list-based LPZK verifier on top of the RAM model. Briefly, this file
  implements the LPZK verifier as a RAM machine, following the RAM formalization given in
  file *ARAM.ec*. 
...
theory RAMVerifier.
  ...
  (** Invokes [gen_f] if the commitment is valid with respect to the circuit *)
  op step_exec (circ : circuit_t) (r : verifier_rand_t) (x : instance_t) (c : commitment_t) =
    let (z, z') = c in
    if valid_z z (add_final_mul circ) then gen_f r z
    else witness.
  ...
  op get_output (st : state_t) : output_t option = 
    if st.`zo <> None then
      ...
      Some (if n <> fzero then get_e f = fmul n (oget st.`r).`alpha else false)
    else None.
