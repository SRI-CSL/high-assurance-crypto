...
theory SecretSharingScheme.
  ...
  op valid_rand : rand_t -> secret_t -> bool.
  op consistent_shares : pid_t -> pid_t -> share_t -> share_t -> bool.
  op valid_shares (ss:shares_t) : bool =
    unzip1 ss = pid_set /\
    all (fun i => all (fun j => consistent_shares i j
                                (get_party_share i ss) (get_party_share j ss)) pid_set) pid_set.
  ...
  op in_rng_share (ws : shares_t) : bool =
    exists r w, valid_rand r (*(secret_public w)*) w /\ valid_secret w /\ share r w = ws.

  axiom correct (r : rand_t) (s : secret_t) :
    valid_rand r (*(secret_public s)*) s =>
  ...
  proof.
    progress.
    have : size (unzip1 (share r s)) = n. 
      rewrite unzip1_share //= pid_set_size => />. 
    by rewrite size_map => />. 
  qed.
  ...
  proof.
    have : size (unzip1 (public_encoding s)) = size pid_set
      by rewrite unzip1_public_encoding => |>. 
    rewrite size_map => /> => H;
      by rewrite H pid_set_size => />. 
  qed.
  ...
  axiom valid_reconstruct ss :
    valid_shares ss => valid_secret (reconstruct ss) /\ valid_rand (reconstruct_rand ss) (*(secret_public (reconstruct ss)*) (reconstruct ss).
  lemma valid_secret_reconstruct ss :
    valid_shares ss => valid_secret (reconstruct ss) by smt(valid_reconstruct).
  lemma valid_rand_reconstruct ss:
    valid_shares ss => valid_rand (reconstruct_rand ss) (*(secret_public (reconstruct ss))*) (reconstruct ss)
      by smt(valid_reconstruct). 
  axiom valid_share r s :
    valid_secret s => valid_rand r (*(secret_public s)*) s => valid_shares (share r s).    
  axiom complete ss :
    valid_shares ss =>
  ...
  lemma public_encoding_rand x :
    valid_secret x => (exists r, valid_rand r (*(secret_public x)*) x /\ share r x = public_encoding x).
  proof.
    move => H. 
    exists (reconstruct_rand (public_encoding x)) => />.
    have : valid_secret (reconstruct (public_encoding x)) /\ valid_rand (reconstruct_rand (public_encoding x)) ((*secret_public*) (reconstruct (public_encoding x))) by rewrite valid_reconstruct //= valid_public_encoding => |>. 
    move => |> H1 H2.
    rewrite public_encoding_correct in H2 => |>.
    rewrite (_:share (reconstruct_rand (public_encoding x)) x = share (reconstruct_rand (public_encoding x)) (reconstruct (public_encoding x))); first by by
      rewrite (public_encoding_correct x) => |>. 
    by rewrite complete => |>; rewrite valid_public_encoding  => |>. 
  qed.
  ...
  proof.
    ...
    have ->: ((exists (r : rand_t) (w : secret_t),
               valid_rand r ((*secret_public*) w) /\ valid_secret w /\ share r w = ss) =
               valid_shares ss) <=> ((exists (r : rand_t) (w : secret_t),
               valid_rand r ((*secret_public*) w) /\ valid_secret w /\ share r w = ss) <=>
               valid_shares ss) by smt().
    split; progress. 
      + by rewrite unzip1_share. 
      + by move : (valid_share r w); rewrite H0 H /= /valid_shares. 
    exists (reconstruct_rand ss) (reconstruct ss) => />. 
    have : valid_secret (reconstruct ss) /\ valid_rand (reconstruct_rand ss) ((*secret_public*) (reconstruct ss)) by rewrite valid_reconstruct => |>. 
    by move => /> H0 H1; rewrite complete => |>. 
  qed.
...
theory ListSecretSharingScheme.
  ...
  lemma size_pids_t_ge0 xs :
    0 <= size_pids_t xs. 
  proof.
    by rewrite /size_pids_t => />; case (onth xs 0) => /> x; rewrite size_ge0 => />. 
  qed.
  ...
  proof.
    rewrite /from_pids_t size_map size_iseq  /size_pids_t => /> H H0. 
    have : size xs = n. 
      have : size (unzip1 xs) = size pid_set by rewrite H => />. 
      by rewrite size_map pid_set_size => />. 
    move => sz.
    rewrite !(onth_nth witness) => />. 
    rewrite sz n_gt0 //=. 
    have H1 : 0 <= size (nth witness xs 0).`2 by rewrite size_ge0.
    by rewrite /max /= /#.
  qed.
  ...
  proof.
    move => Sz. 
    rewrite /size_pids_t /to_pids_t => />. 
    rewrite onth_map /get_party_share => />.
    rewrite (onth_nth witness) => />. 
    rewrite !size_map => />. 
  qed.
...
proof.
    ...
    have : size xs = n. have : size (unzip1 xs) = size pid_set. rewrite H => />. rewrite size_map pid_set_size => />. move => sz.
  ...
  lemma from_to_pids_t xs :
    ...
    move => H. apply (eq_from_nth witness) => />. rewrite size_from_pids_t => |>. rewrite valid_to_pids_t => |>. rewrite size_to_pids_t => |>. rewrite pid_set_size => />. rewrite n_gt0 => />. move => i. rewrite size_from_pids_t => |>. rewrite valid_to_pids_t => |>. rewrite size_to_pids_t => |>. rewrite pid_set_size //. rewrite n_gt0 //. move => Hi1 Hi2.  
    rewrite /from_pids_t /to_pids_t => |>. rewrite !(nth_map witness) => |>. rewrite size_iseq => |>. rewrite ge0_max => |>. rewrite size_pids_t_ge0 => |>. rewrite /size_pids_t onth_map omap_some => |>. rewrite (onth_nth witness) => />. rewrite pid_set_size n_gt0 => />. rewrite size_map => />. rewrite -!map_comp /(\o) => />.
    apply (eq_from_nth witness) => |>. rewrite size_map => |>. rewrite pid_set_size => />. move :H. rewrite allP /valid_shares => |>H. have : unzip1 (nth witness xs i) = pid_set /\ all (fun (i0 : pid_t) => all (fun (j : pid_t) => consistent_shares i0 j (get_party_share i0 (nth witness xs i)) (get_party_share j (nth witness xs i))) pid_set) pid_set. rewrite H => |>. rewrite nth_in => |>. move => |> H0 H1. have : size (unzip1 (nth witness xs i)) = size pid_set. rewrite H0 => />. rewrite size_map pid_set_size => />. move => H2. rewrite H2 => />.
    move => j. rewrite size_map => />Hj1 Hj2. rewrite !(nth_map witness) => />. rewrite !nth_iseq => />. rewrite /size_pids_t !onth_map /prod /idfun => />. rewrite !(onth_nth witness) => />. rewrite pid_set_size n_gt0 => />. rewrite size_map andaP => />. rewrite !(nth_map witness) /get_party_share => />.
    rewrite eq2 => />. rewrite (unzip1_eq_nth _ pid_set) => />. move :H. rewrite allP /valid_shares => |>H. have : unzip1 (nth witness xs i) = pid_set /\ all (fun (i0 : pid_t) => all (fun (j0 : pid_t) => consistent_shares i0 j0 (get_party_share i0 (nth witness xs i)) (get_party_share j0 (nth witness xs i))) pid_set) pid_set. rewrite H => />. rewrite nth_in => />. move => /> H0 H1.  have : size (unzip1 (nth witness xs i)) = size pid_set. rewrite H0 => />. rewrite size_map pid_set_size => />. move => H4. rewrite H4 => />. move :Hj2. rewrite pid_set_size => />. 
    ...
    rewrite -H => />. rewrite !(nth_map witness) => />. have : size (unzip1 (nth witness xs i)) = size pid_set. rewrite H => />. rewrite size_map pid_set_size => />. move => H4. rewrite H4 => />. move :Hj2. rewrite pid_set_size => />. rewrite assoc_nth_some => />. have : size (unzip1 (nth witness xs i)) = size pid_set. rewrite H => />. rewrite size_map pid_set_size => />. move => H4. rewrite H4 => />. move :Hj2. rewrite pid_set_size => />. rewrite H pid_set_uniq => />. qed.
  ...
  lemma to_from_pids_t xs :
    ...
    have : size xs = n. have : size (unzip1 xs) = size pid_set. rewrite H => />. rewrite size_map => />. rewrite pid_set_size => />. move => sz.
    apply (eq_from_nth witness) => />. rewrite size_map => />. rewrite pid_set_size sz => />. 
    move => i. rewrite size_map => />. rewrite pid_set_size => />Hi1 Hi2. 
    rewrite !(nth_map witness) => />. rewrite pid_set_size => />. rewrite -!map_comp /(\o) /get_party_share => />. rewrite eq2 => />. rewrite -H !(nth_map witness) => />. rewrite sz => />.
  ...
  clone include SecretSharingScheme with
    ...
    have : size r = size s. move :H. rewrite /valid_rand /secret_public => |>. move => Sz.
    ...
    pose pp := ((nth witness r i), (nth witness s i)). have : valid_rand pp.`1 pp.`2. rewrite H3 => |>. rewrite /pp in_nth => |>. exists i => |>. rewrite size_zip !(nth_onth) !(onth_nth (witness,witness)) => |>. rewrite size_zip => />. rewrite Sz min_eq => />. rewrite Sz min_eq => />. rewrite nth_zip => |>. rewrite !(onth_nth witness) => |>. rewrite /pp => |>. have : false. smt(). progress.
    ...
    move :H. rewrite /valid_rand /secret_public allP => |>Sz' H3. case (i < size r) => |>H4.
    pose pp := (nth witness r i, (nth witness s i)). have : valid_rand pp.`1 pp.`2. 
    rewrite H3 => |>. rewrite in_nth /pp => |>. exists i => |>. rewrite size_zip Sz min_eq => |>. rewrite !(nth_onth) !(onth_nth (witness,witness)) => |>. rewrite size_zip Sz min_eq => 
|>. rewrite nth_zip => |>. rewrite !(onth_nth witness) => |>. rewrite /pp => |>. have : false. smt(). progress.
    ...
    have : size x = size y.
    have : map (SS.get_party_share (nth witness pid_set 0)) (map SS.public_encoding x) = map (SS.get_party_share (nth witness pid_set 0)) (map SS.public_encoding y). rewrite H => />. rewrite nth_in => />. rewrite pid_set_size n_gt0 => />. rewrite -!map_comp /(\o) /get_party_share => />H1. clear H. have : size (map (fun (x0 : SS.secret_t) => oget (assoc (public_encoding x0) (nth witness ListSecretSharingScheme.pid_set 0))) x) = size (map (fun (x0 : SS.secret_t) => oget (assoc (public_encoding x0) (nth witness ListSecretSharingScheme.pid_set 0))) y). rewrite H1 => />. rewrite !size_map => />. move => Sz.
    ...
    have : map (SS.get_party_share pid1) (map SS.public_encoding x) = map (SS.get_party_share pid1) (map SS.public_encoding y). rewrite H => />. rewrite /pid1 => />. rewrite (unzip1_eq_nth _ SS.pid_set) => />. rewrite SS.size_public_encoding => />. rewrite nth_in => />. rewrite pid_set_size => />. 
    ...
    rewrite eq2 => />. qed.
    realize valid_share. move => r ss. rewrite /valid_secret /valid_rand /secret_public /valid_shares /share !allP /to_pids_t => />H H0. rewrite -!map_comp /(\o) => />. rewrite map_id => />. move => H1 i Hi.
    ...
    have : SS.valid_shares (share (nth witness r n) (nth witness ss n)). rewrite SS.valid_share => />. rewrite H => />. rewrite nth_in => />. case (n < size r) => />H2. have : valid_rand (nth witness r n, (nth witness ss n)).`1  (nth witness r n, (nth witness ss n)).`2. rewrite H1 => />. rewrite in_nth => />. exists n => />. rewrite size_zip H0 min_eq => />. rewrite !nth_onth !(onth_nth (witness,witness)) => />. rewrite size_zip => />. rewrite H0 min_eq => />. rewrite nth_zip => />. rewrite !(onth_nth witness) => />. progress. have : false. smt(). progress.
    ...
    rewrite !nth_zip => |>. rewrite !size_map => |>. rewrite !(nth_map witness) => |>. rewrite size_from_pids_t => |>. rewrite size_iseq => />. rewrite ge0_max => />. rewrite size_pids_t_ge0 => />. rewrite size_from_pids_t => />. rewrite size_iseq => />. rewrite ge0_max => />. rewrite size_pids_t_ge0 => />. rewrite !nth_iseq Hi1 Hi2 => />. 
    ...
    have : size ss = n. have : size (unzip1 ss) = size (ListSecretSharingScheme.pid_set). move :H. rewrite /valid_shares => />H H0. rewrite H => />. rewrite size_map pid_set_size => />. move => H1.
