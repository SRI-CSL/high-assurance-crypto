...
module Sha256 (PC : PartyConfiguration) = struct 
  ...
  module BGWData = BGWData (PC)
  
  type input_t = BGWData.view_t
  type output_t = string
  type key_t = unit

  let f (k : key_t) (x : input_t) : output_t = 
    let sha256 = Cryptokit.Hash.sha256 () in
    sha256#add_string (BGWData.view_to_string x);
...
module BGWBool (PC : PartyConfiguration) = struct
  ...
  let get_messages_to = BGW.get_messages_to
  let get_messages_from = BGW.get_messages_from
...
module ShamirBGWSha256InTheHead (PC : PartyConfiguration) = InTheHead (InTheHeadZKData) (ListSecretSharing (Shamir (PC))) (BGWBool (PC)) (CRPRFCommitment (Sha256 (PC)))
