...
module BGWRefreshData (PC : PartyConfiguration) = struct
  ...
  let party_exec2 (x : input_t) (ms : (pid_t * S.share_t) list) : poutput_t = fadd (fst (snd x)) (sumt (map snd ms))
...
module BGWRefreshGate (PC : PartyConfiguration) = Gate (BGWRefreshData (PC))
