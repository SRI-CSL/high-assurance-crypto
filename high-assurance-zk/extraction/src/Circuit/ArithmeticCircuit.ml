...
module ArithmeticGates = struct
  ...
  let rec valid_gids (np : Z.t) (ns : Z.t) q (g : gates_t) : bool =
    match g with
    | PInput _ -> true
    | SInput _ -> true
    | Constant _ -> true
    | Addition (gid, wl, wr) -> Z.leq (Z.add np ns) gid && Z.lt gid (Z.add np (Z.add ns q)) && valid_gids q np ns wl && valid_gids q np ns wr
    | Multiplication (gid, wl, wr) -> Z.leq (Z.add np ns) gid && Z.lt gid (Z.add np (Z.add ns q)) && valid_gids q np ns wl && valid_gids q np ns wr
    | SMultiplication (gid, wl, wr) -> Z.leq (Z.add np ns) gid && Z.lt gid (Z.add np (Z.add ns q)) && valid_gids q np ns wl && valid_gids q np ns wr
