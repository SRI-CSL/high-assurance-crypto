...
theory InputFunctionality.
  ...
  op valid_inputs (xs : inputs_t) : bool =
    ...
    (forall pid, pid \in pid_set => valid_secret (sinput pid xs)). 
  ...
  op f (xs : inputs_t) : outputs_t = 
    ...
    let sec = sumt (map snd ssec) in
    map (fun pid => (pid, sec)) pid_set.
