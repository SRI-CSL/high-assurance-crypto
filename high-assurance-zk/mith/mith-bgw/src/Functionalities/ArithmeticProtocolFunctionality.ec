...
theory ArithmeticProtocolFunctionality.
  ...
  op valid_inputs_topology (topo : topology_t) (xs : inputs_t) : bool =
    ...
    (forall k, np <= k < np+ns => 
      ...
      valid_secret (sumt (map snd sec))) /\ 
    (forall k, np <= k < np+ns => 
      let sec = map (fun pid => (pid, nth witness (sinput pid xs) k)) pid_set in
      (forall pid, pid \in pid_set => 
                                      valid_secret (oget (assoc sec pid)))). 
  ...
  op f (c : circuit_t) (xs : inputs_t) : outputs_t =
    ...
    let secs = map (fun sec => (sumt (map snd sec))) secs in
  ...
  clone import ProtocolFunctionality with
    type circuit_t = ArithmeticCircuit.Circuit.circuit_t,
    op n = n,
    op t = t,
    type pid_t = pid_t,
    op pid_set = pid_set,
