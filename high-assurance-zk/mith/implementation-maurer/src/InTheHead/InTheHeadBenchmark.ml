...
let mpc_time = ref 0.0
let pcommit_time = ref 0.0
let challenge_time = ref 0.0
let response_time = ref 0.0
let vcommit_time = ref 0.0
let check_time = ref 0.0
...
module InTheHeadSigmaProtocolData
         (ZKD : ZKData)
         (SSD : sig include SecretSharingSchemeData with type secret_t = ZKD.witness_t end)
         (MPCD : sig include ProtocolData with type pid_t = SSD.pid_t and 
                 type pinput_t = ZKD.instance_t and type sinput_t = SSD.share_t and type output_t = bool end)
  ...
  module MPC = Protocol (MPCD)
  module CS = CommitmentScheme(CSD)

  type witness_t = ZKD.witness_t
  type statement_t = ZKD.instance_t

  type prover_input_t = ZKD.witness_t * ZKD.instance_t
  type verifier_input_t = ZKD.instance_t
  type inputs_t = prover_input_t * verifier_input_t

  type prover_rand_t = SS.rand_t * MPC.rands_t * (SSD.pid_t * CS.rand_t) list
  ...
  type committed_views_t = (SSD.pid_t * committed_view_t) list

  let get_committed_view (pid : SSD.pid_t) (cvs : committed_views_t) : committed_view_t = oget (assoc cvs pid)

  type commitment_t = (SSD.pid_t * CS.commitment_t) list

  let get_party_commitment (pid : SSD.pid_t) (cv : commitment_t) : CS.commitment_t = oget (assoc cv pid)
  ...
  let commitment (rp : prover_rand_t) (xp : prover_input_t) : prover_state_t * commitment_t =
    ...
    let x_mpc = MPC.mk_inputs x ws in
  ...
  let check (stv : verifier_state_t) (r : response_t) : bool =
    ...
    let cvi = CS.verify vi (ci,osi) in
    let cvj = CS.verify vj (cj,osj) in
    
    let t1 = Sys.time () in

    vcommit_time := (t1 -. t0) *. 1000. ;

    let t0 = Sys.time () in

    let cvvs = MPC.consistent_views_public_output relc xv vi vj i j in
    
    let t1 = Sys.time () in
    check_time := (t1 -. t0) *. 1000. ;

    (* Format.printf "cvi = %s | cvj = %s | cvvs = %s@." (if cvi then "TRUE" else "FALSE") (if cvj then "TRUE" else "FALSE") (if cvvs then "TRUE" else "FALSE") ; *)
...
module InTheHeadBenchmark
         (ZKD : ZKData)
         (SSD : sig include SecretSharingSchemeData with type secret_t = ZKD.witness_t end)
         (MPCD : sig include ProtocolData with type pid_t = SSD.pid_t and 
                 type pinput_t = ZKD.instance_t and type sinput_t = SSD.share_t and type output_t = bool end)
         (CSD : sig include CommitmentSchemeData with type msg_t = MPCD.view_t end) = 
                 InTheHeadSigmaProtocolData (ZKD) (SSD) (MPCD) (CSD)
                                                        
(*module InTheHead
         (ZKD : ZKData)
         (SSD : sig include SecretSharingSchemeData with type secret_t = ZKD.witness_t end)
         (MPCD : sig include ProtocolData with type pid_t = SSD.pid_t and type pinput_t = ZKD.instance_t and type sinput_t = SSD.share_t and type output_t = bool end)
         (RELC : sig val relc : MPCD.circuit_t end)
         (CSD : sig include CommitmentSchemeData with type msg_t = MPCD.view_t end) = ZKSigmaProtocol (InTheHeadSigmaProtocolData (ZKD) (SSD) (MPCD) (CSD))*)
