...
module ConstantGate = struct
  ...
  let empty_trace : trace_t = ((), (), (), (), ())

  let party_exec (r : rand_t) (x : input_t * input_t) : output_t = let (x1,x2) = x in x1
  ...
  let local_party_exec (x : input_t * input_t) (r : rand_t) (tr : trace_t) : output_t = let (x1,x2) = x in x1
