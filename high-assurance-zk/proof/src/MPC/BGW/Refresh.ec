...
progress.
    ...
    have ->: (map (fun (pid : SecretSharingScheme.pid_t) => (pid, fadd (oget (assoc xs pid)).`2.`1 (sumt (unzip2 (oget (assoc (map (fun (pid0 : pid_t) => (pid0, map (fun (ms : pid_t * BGWRefresh.out_messages_t) => let (sender, m) = ms in (sender, (BGWRefresh.get_messages_to pid0 m))) (map (fun (pid1 : pid_t) => (pid1, map (fun (pid2 : coeficient) => (pid2, eval pid2 (update (oget (assoc rs pid1)) 0 fzero))) pid_set)) pid_set))) pid_set) pid)))))) pid_set) 
=
(map (fun (pid : SecretSharingScheme.pid_t) => (pid, fadd (oget (assoc xs pid)).`2.`1 (sumt (unzip2 (oget (assoc (map (fun (pid0 : pid_t) => (pid0, map ((fun (ms : pid_t * BGWRefresh.out_messages_t) => let (sender, m) = ms in (sender, (BGWRefresh.get_messages_to pid0 m))) \o (fun (pid1 : pid_t) => (pid1, map (fun (pid2 : coeficient) => (pid2, eval pid2 (update (oget (assoc rs pid1)) 0 fzero))) pid_set))) pid_set)) pid_set) pid)))))) pid_set).
...
have ->: interpolate fzero (map (fun (pid : SecretSharingScheme.pid_t) => (pid, (oget (assoc xs pid)).`2.`1)) pid_set) = interpolate fzero (map (fun (pid : SecretSharingScheme.pid_t) => (pid, eval pid (update r 0 s))) pid_set).
congr.
rewrite -eq_in_map; progress.
by rewrite H3 /=.
rewrite interpolate_eval.
...
rewrite !map_assoc //=.
  ...
  op simulator (rs : rands_t) (xs : inputs_t) (cr : pid_t list) (ys : outputs_t) : views_t =
    ...
    let poutsc = map (fun pid => (pid, oget (assoc pouts pid))) cr in
