...
from BGW require import Addition Multiplication SMultiplication.
...
(*    op SecretSharingScheme.n = HMShamir.n,
  ...
  realize ArithmeticProtocolFunctionality.SecretSharingScheme.t_pos by apply t_pos.
  ...
  realize ArithmeticProtocolFunctionality.ProtocolFunctionality.t_pos by apply t_pos.
  ...
  op simulate_gates (gg : gates_t) (rs : rands_t) (xs : inputs_t) (cr : pid_t list) : poutputs_t * (pid_t * (ProtocolFunctionality.input_t * rand_t * gate_local_traces_t)) list =
    ...
    with gg = SInput w => 
      let ys = map (fun pid => let sec = nth witness (ProtocolFunctionality.sinput pid xs) w in (pid, sec)) cr in
      let tr = map (fun pid => (pid, (ProtocolFunctionality.input pid xs, ArithP.rand pid rs, SInputLT w))) cr in
    ...
    with gg = SMultiplication gid wl wr => 
      ...
      let gxs = map (fun pid => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) cr in
  ...
  clone import WeakSecurity with
    ...
    op get_local_rand (pid : pid_t) (r : gate_rands_t) : gate_rand_t =
      ...
      with r = MultiplicationRands rm => MultiplicationRand (rand pid rm)
      ...
      equiv [ RP(BGWAddition.R, BGWMultiplication.R, BGWSMultiplication.R).gen ~
              RP(BGWAddition.R, BGWMultiplication.R, BGWSMultiplication.R).gen : 
                ...
                ={c, aux, cr} /\ cr_ = cr{2} /\ c_ = c{2} /\ 
                  ...
                  ==> ={glob RP} /\ unzip1 res{1} = unzip1 res{2} /\ unzip1 res{1} = range (c_.`1.`1 + c_.`1.`2) (c_.`1.`1 + c_.`1.`2 + c_.`1.`4) /\
    ...
    module R (RP : RandParam_t) = {
      proc gen(c : circuit_t, cr : pid_t list, aux : aux_t) : rands_t = {
        ...
        rs <- map (fun pid => (pid, map (fun gid => (gid, get_local_rand pid (oget (assoc grs gid)))) (range (c.`1.`1 + c.`1.`2) (c.`1.`1 + c.`1.`2 + c.`1.`4)))) ProtocolFunctionality.pid_set;
...
seq 1 1 : (#pre /\ unzip1 grs{1} = unzip1 grs{2} /\ unzip1 grs{1} = range (c_.`1.`1 + c_.`1.`2) (c_.`1.`1 + c_.`1.`2 + c_.`1.`4) /\
...
rewrite H6 /=.
by move : H31; rewrite !map_assoc; first by rewrite mem_range => /#. 
...
move : H31; rewrite !map_assoc //=; first by rewrite mem_range => /#.
rewrite (get_local_rand_mul (oget (assoc grs{1} gid)) pid).
smt().
done.
progress.
move : (H44 pid); rewrite H6 /=.
...
rewrite /simulator /=.
move : H2 H4 H5 H6 H7 H8; elim (c_) => topo gg /=.
elim topo => np ns m q /=.
...
rewrite /valid_rand /=.
rewrite -map_comp /(\o) /= map_id /=.
move => /> H2 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19. 
rewrite /protocol /= /eval_circuit /= /trace /=.
move : H9 H10 H11 H12 H19.
elim gg => //=.
...
progress.
move : H10; rewrite allP; progress.
               ...
               (fun (pid0 : pid_t) =>
                  (pid0,
                   map (fun (gid0 : gid_t) => (gid0, get_local_rand pid0 (oget (assoc grs{1} gid0))))
                     (range (np + ns) (np + ns + q)))) ProtocolFunctionality.pid_set) pid))) <=> true.
progress.
move : (H31 pid); rewrite H9 /=.
by done.
        ...
        (fun (pid : pid_t) =>
           (pid,
            map (fun (gid0 : gid_t) => (gid0, get_local_rand pid (oget (assoc grs{2} gid0))))
              (range (np + ns) (np + ns + q)))) ProtocolFunctionality.pid_set).
pose rs := (map
        (fun (pid : pid_t) =>
           (pid,
...
smt().
smt().
rewrite (H39 x0 (fmul (oget (assoc vwl x1)) (oget (assoc vwr x1))) (fmul fzero fzero)).
