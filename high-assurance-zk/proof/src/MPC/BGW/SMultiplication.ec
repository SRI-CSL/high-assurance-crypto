...
theory BGWSMultiplication.
  ...
  op party_exec (pid : pid_t) (r : rand_t) (x : input_t) : output_t = 
    let (pub, sec) = x in fmul (pub_reconstruct pid pub) sec.`1.
...
by rewrite /gate /eval /=.
  ...
  realize Gate.correct.
    ...
    progress.
      ...
      move : H4; rewrite /share /=.
      rewrite -eq_in_map /sinput /input; progress.
    rewrite /pub_reconstruct /= /pinput /input /=.
    have ->: (map (fun (pid0 : pid_t) => (pid0, let (pub, sec) = oget (assoc xs pid0) in fmul pub sec.`1)) pid_set) = (map (fun (pid : pid_t) => (pid, eval pid (smul (oget (assoc xs (head witness Shamir.pid_set))).`1 (update r 0 s)))) pid_set).
      rewrite -eq_in_map /=; progress; rewrite eval_smul.
      by move : H5; rewrite /public_encoding /share /= -!eq_in_map /= /#.
      rewrite /reconstruct.
      * rewrite (interpolate_eval pid_set (smul (oget (assoc xs (head witness Shamir.pid_set))).`1 (update r 0 s))).
          by apply pid_set_uniq.
          by rewrite degree_smul !degree_update0 H1 pid_set_size /max /=; smt(t_pos).
...
progress.
move : x1 x2 H0 => tr ys; progress.

have ->: ys = (Gate.gate rs xs).`2 by smt().
have ->: tr = (Gate.gate rs xs).`1 by smt().
clear H0.
rewrite /gate /= /output /eval /= /party_exec.
rewrite !map_assoc //=.
rewrite /local_output /=.
rewrite /trace /=.
rewrite !map_assoc //=.
rewrite !map_assoc //=.
rewrite /local_output_share /input /=.
...
rewrite -H0 /=.
  ...
  realize Gate.messages_consistency.
    by rewrite /valid_input /valid_inputs /valid_rand /valid_rands /gate /out_messages
  ...
  realize eval_preserves_share.
    rewrite /valid_inputs /valid_input /valid_inputs /valid_rands /valid_rand /output_pred
            /input_pred /gate /eval /party_exec /f /sum_pair /reconstruct /o /=.
progress.
exists (smul s0 (update r 0 s)).
        rewrite /valid_rand degree_smul degree_update0 size_smul size_update0 H1 H2; progress.
          rewrite smul_expo; first by rewrite size_update0 H2 //.
          rewrite update_expo; first by rewrite H2 //.
          by rewrite H3.
        rewrite /share /= -eq_in_map /=; progress.
        rewrite eval_update //. 
          rewrite expo_diff_count //.
            by rewrite size_smul size_update0 H2; smt. 
            rewrite size_smul size_update0 H2; progress.
            rewrite smul_expo; first by rewrite size_update0 H2 //.
            rewrite update_expo; first by rewrite H2 //.
            by rewrite H3 //.
        rewrite get_monomial0.
          rewrite expo_diff_count //; first by rewrite size_smul size_update0 H2; smt. 
          rewrite size_smul size_update0 H2; progress.
          rewrite smul_expo; first by rewrite size_update0 H2 //.
          rewrite update_expo; first by rewrite H2 //.
          by rewrite H3 //.
      ...
      rewrite -eq_in_map /sinput /input; progress.
    rewrite /pub_reconstruct /= /pinput /input /=.
    have ->: (map (fun (pid0 : pid_t) => (pid0, let (pub, sec) = oget (assoc xs pid0) in fmul pub sec.`1)) pid_set) = (map (fun (pid : pid_t) => (pid, eval pid (smul (oget (assoc xs x)).`1 (update r 0 s)))) pid_set).
      rewrite -eq_in_map /=; progress; rewrite eval_smul.
      by move : H5; rewrite /public_encoding /share /= -!eq_in_map /= /#.
      rewrite /reconstruct.
...
rewrite  -H8 /=.
rewrite H4 //=.
have ->: (oget (assoc xs x)).`2.`1 = sec.`1 by smt().
have ->: pub = s0.
move : H5; rewrite /pinput /= /input /=.
