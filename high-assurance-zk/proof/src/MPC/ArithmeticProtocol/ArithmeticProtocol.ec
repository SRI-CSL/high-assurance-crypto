...
from ArithmeticProtocol require import AdditionGate MultiplicationGate SMultiplicationGate.
...
theory ArithmeticProtocol.
  ...
  proof. 
    ...
    elim topo => np ns m q /=.
    rewrite !allP /=.
    elim fx => fxp fxs => /> H H0 H1 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16.
    move : H13 H7 H6 H5 H4; elim gg.
      move => /> w H18 H19.
split.
...
smt().
    ...
    move => /> w H18 H19.
    move : (H16 w); rewrite H18 H19 /=.
    rewrite /sinput /input /= -map_comp /(\o) /= map_id /=.
    move => H21 /=; split; first by smt().
    move : (H12 w); rewrite H18 H19 /=; progress.
rewrite H21 -H6 /=.
move : H5 (correct r s).
rewrite /valid_rand => H5; rewrite H4 H5 /=.
smt().

    by exists r; rewrite H5 /=; smt. 
...
have ->: (AdditionGate.valid_inputs
    (map
       (fun (pid : AdditionFunctionality.SecretSharingScheme.pid_t) =>
          (pid, (tt, (oget (assoc vwl pid), oget (assoc vwr pid))))) pid_set)).
...
move : (MultiplicationGate.eval_preserves_share (map (fun (pid : pid_t) => (pid, as_gate_rand_multiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map
          (fun (pid : AdditionFunctionality.SecretSharingScheme.pid_t) =>
             (pid, (tt, (oget (assoc vwl pid), oget (assoc vwr pid))))) pid_set)).
have ->: (MultiplicationGate.valid_inputs
    (map
       (fun (pid : AdditionFunctionality.SecretSharingScheme.pid_t) =>
          (pid, (tt, (oget (assoc vwl pid), oget (assoc vwr pid))))) pid_set)).
rewrite /valid_inputs.
...
smt(@List n_pos pid_set_size).
    ...
    have H32 : exists trr vwr, (trr, vwr) = eval_gates wr rs xs.
      by exists (eval_gates wr rs xs).`1 (eval_gates wr rs xs).`2 => /#.
    elim H32 => /> trr vwr H32; rewrite -H32 /=.
    move : (SMultiplicationGate.Gate.correct (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map (fun (pid : pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).
    rewrite /input_pred /sinput /input /pid_set /reconstruct /valid_inputs /pid_set 
            -map_comp /(\o) map_id /valid_secret /valid_rand /share /sinput /input
            /valid_rands /pid_set -map_comp /(\o) map_id /output_pred 
            /pid_set /reconstruct /gate /f /to_foutput /sinput /input /reconstruct /=.
    have ->: reconstruct (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc (map (fun (pid : pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness<:share_t>)))) pid_set) pid)).`2.`1)) SMultiplicationFunctionality.SecretSharingScheme.pid_set) = (eval_gates wr (fxp, fxs)).
      have <-: reconstruct (eval_gates wr rs xs).`2 = (eval_gates wr (fxp, fxs)) by smt(). 
      have ->: (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc (map (fun (pid : pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness<:share_t>)))) pid_set) pid)).`2.`1)) SMultiplicationFunctionality.SecretSharingScheme.pid_set) = (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, ((oget (assoc vwr pid))))) pid_set).
        rewrite -eq_in_map; progress.
        by rewrite map_assoc //=.
      have H33 : unzip1 (eval_gates wr rs xs).`2 = pid_set by smt().
      ...
      by rewrite fun_ext /(==) => /#.
    simplify.
    have H50 : (exists (s : SMultiplicationFunctionality.SecretSharingScheme.secret_t), map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (pinput pid (map (fun (pid0 : pid_t) => (pid0, (oget (assoc vwl pid0), (oget (assoc vwr pid0), witness)))) pid_set)))) pid_set = public_encoding s).
...
by rewrite !map_assoc //=.
    simplify.
    have ->: (exists (r : SMultiplicationFunctionality.SecretSharingScheme.rand_t) (s : SMultiplicationFunctionality.SecretSharingScheme.secret_t), valid_secret s /\ valid_rand r /\ share r s = map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc (map (fun (pid0 : pid_t) => (pid0, (oget (assoc vwl pid0), (oget (assoc vwr pid0), witness<:share_t>)))) pid_set) pid)).`2.`1)) pid_set).
      have H33 : exists (r : ArithmeticProtocol.SecretSharingScheme.rand_t),
                          (valid_rand r)%SecretSharingScheme /\
                          share r (eval_gates wr (fxp, fxs)) = (eval_gates wr rs xs).`2 by smt().
      elim H33 => /> r H33 H34.
      exists r (eval_gates wr (fxp, fxs)).
      rewrite H33 H34 /=.
      have ->: map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc (map (fun (pid0 : pid_t) => (pid0, (oget (assoc vwl pid0), (oget (assoc vwr pid0), witness<:share_t>)))) pid_set) pid)).`2.`1)) pid_set = (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, ((oget (assoc vwr pid))))) pid_set).
        rewrite -eq_in_map; progress.
        by rewrite map_assoc.
      have H35 : unzip1 (eval_gates wr rs xs).`2 = pid_set by smt().
      rewrite (eq_assoc_map (eval_gates wr rs xs).`2); first by rewrite H35 pid_set_uniq.
    ...
    move : (SMultiplicationGate.Gate.correct_domain (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)). 
    have H34 : exists (gtr, gys), (gtr, gys) = eval (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set).
      by exists (eval (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).`1
                (eval (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).`2 => /#.
    elim H34 => gtr gys H34; rewrite -H34 /=.
    have ->: unzip1 gys = pid_set.
      have ->: gys = (eval (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).`2 by smt().
      by move : (SMultiplicationGate.eval_domain (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)) => /#. 
  ...
  ((gate (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set)
      (map
         (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) =>
            (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)))%AdditionGate.Gate.`2 = SMultiplicationGate.Gate.GateFunctionality.pid_set.
smt.
have ->: unzip1
  ((gate (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set)
      (map
         (fun (pid : SMultiplicationFunctionality.SecretSharingScheme.pid_t) =>
            (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)))%AdditionGate.Gate.`1 =
...
move : (SMultiplicationGate.eval_preserves_share (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) gid)))) pid_set) (map
          (fun (pid : AdditionFunctionality.SecretSharingScheme.pid_t) =>
             (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).

have ->: (SMultiplicationGate.valid_inputs
    (map
       (fun (pid : AdditionFunctionality.SecretSharingScheme.pid_t) =>
          (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).
rewrite /valid_inputs.
...
qed.
  ...
  proof.
    ...
    move : (correct_eval_gates (np,ns,m,q) gg rs xs ((oget (assoc xs (head witness pid_set))).`1, map (fun (x : int) => ((reconstruct (map (fun (pid0 : SecretSharingScheme.pid_t) => (pid0, nth witness (oget (assoc xs pid0)).`2 x)) pid_set)))%SecretSharingScheme) (range 0 (np+ns)))).
    rewrite H.
    ...
    have ->: valid_inputs_topology (np, ns, m, q) xs. 
      ...
      move : (H7 k); rewrite H12 H13 /=; progress => /#.
      move : (H9 k); rewrite H12 H13 /= /sinput /input /=; progress.
      by exists r s => /#.
      have ->: all (fun (pid : pid_t) => gates_valid_rand gg (oget (assoc rs pid))) pid_set by rewrite allP.
    have ->: pinput_finput_valid (((np, ns, m, q), witness), xs, ((oget (assoc xs (head witness pid_set))).`1, map (fun (x : int) => (reconstruct (map (fun (pid0 : SecretSharingScheme.pid_t) => (pid0, nth witness (oget (assoc xs pid0)).`2 x)) pid_set))) (range 0 (np+ns)))).
      ...
      rewrite /finput_party_input_tuple_valid /=; split.
        ...
        move : (H8 k); rewrite H12 H13 /=.
        progress; move : H15; rewrite allP; progress.
        move : (H15 (pid, nth witness (oget (assoc xs pid)).`1 k)). 
        rewrite mapP.
        have ->: (exists (x : SecretSharingScheme.pid_t), (x \in pid_set) /\ (pid, nth witness (oget (assoc xs pid)).`1 k) = (fun (pid1 : SecretSharingScheme.pid_t) => (pid1, nth witness (oget (assoc xs pid1)).`1 k)) x)
          by exists pid; rewrite H14 => //.
        simplify => H18.
        move : (H15 ((head witness pid_set), 
        ...
        have ->: (exists (x : SecretSharingScheme.pid_t), (x \in pid_set) /\ (head witness pid_set, nth fzero (oget (assoc xs (head witness pid_set))).`1 k) = (fun (pid1 : SecretSharingScheme.pid_t) => (pid1, nth witness (oget (assoc xs pid1)).`1 k)) x).
          ...
          by smt(@List).
        by smt().
      progress; move : (H9 k); rewrite H12 H13 /=; progress.
      rewrite -H16 /= (nth_map witness fzero); first by rewrite size_range /max /= /#.
      simplify.
      have ->: (nth witness (range 0 (np+ns)) k) = k by smt(@Range).
      rewrite -H16; move : (SecretSharingScheme.correct r s).
      by rewrite /valid_rand H15 /valid_secret /= => /#.
    rewrite /pgates_fgates_valid /=; progress; rewrite -H12.
...
smt().
  ...
  op local_output_gates (pid : pid_t) (x : input_t) 
    ...
    with im = SInputLT w => nth witness x.`2 w
    ...
    with im = SMultiplicationLT gid tr trl trr => 
      ...
      SMultiplicationGate.local_output_share pid (vl,(vr,witness)) rm tr.
  ...
  proof.
    move => hpid; elim g.
      ...
      * move => /= gid wl wr H H0.
        ...
        have H2 : exists tr vwr, (tr,vwr) = eval_gates wr rs xs by exists (eval_gates wr rs xs).`1 (eval_gates wr rs xs).`2 => /#.
        elim H2 => tr vwr H2; rewrite -H2.
        move : (SMultiplicationGate.local_output_share_correct pid (map (fun (pid0 : SMultiplicationFunctionality.SecretSharingScheme.pid_t) => (pid0, (oget (assoc vwl pid0), (oget (assoc vwr pid0), witness)))) pid_set) (map (fun (pid0 : pid_t) => (pid0, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid0)) gid)))) pid_set)).
        by rewrite /pid_set /= /gate /= /output /input /rand 
                /local_output !map_assoc //= hpid /= => /#. 
  qed.
...
by rewrite H /= /#.
  ...
  op out_messages_gates (pid : pid_t) (x : input_t) 
    ...
    with im = SMultiplicationLT gid tr trl trr => 
      ...
      let rm = as_gate_rand_smultiplication (oget (assoc r gid)) in
      SMultiplicationLT gid (SMultiplicationGate.out_messages_in_messages pid (vl,(vr,witness)) rm tr) owl owr.
  ...
  lemma messages_gates_consistency (g : gates_t) (i j : pid_t) (xs : inputs_t) (rs : rands_t) :
  ...
  proof.
    progress; elim g.
      ...
      * move => w wl wr; progress.
        ...
        have H4 : exists tr vwr, (tr,vwr) = eval_gates wr rs xs by exists (eval_gates wr rs xs).`1 (eval_gates wr rs xs).`2 => /#.
        elim H4 => tr vwr H4; rewrite -H4 /=.
        move : (SMultiplicationGate.Gate.messages_consistency i j (map (fun (pid : pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set) (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) w)))) pid_set)).
        rewrite /pid_set /gate /= H H0 /= /input /rand /trace
                !map_assoc //=.
        have : exists (tr0, ys), (tr0, ys) = eval (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) w)))) pid_set) (map (fun (pid : pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set) 
          by exists (eval (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) w)))) pid_set) (map (fun (pid : pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).`1
                    (eval (map (fun (pid : pid_t) => (pid, as_gate_rand_smultiplication (oget (assoc (oget (assoc rs pid)) w)))) pid_set) (map (fun (pid : pid_t) => (pid, (oget (assoc vwl pid), (oget (assoc vwr pid), witness)))) pid_set)).`2 => /#.
        progress; move : H6; rewrite -H5 /=.
        rewrite !map_assoc //= /out_messages /get_messages_to /get_messages_from.
        move : (local_output_correct_gates witness wl i xs rs); rewrite H /=.
        move : (local_output_correct_gates witness wr i xs rs); rewrite H /=.
