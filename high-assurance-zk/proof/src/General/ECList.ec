...
(*                            Transpose                                 *)
(* -------------------------------------------------------------------- *)
op row (xs : 'a list list) (i : int) : 'a list = nth [] xs i.
op col (xs : 'a list list) (j : int) z : 'a list = map (fun x => nth z x j) xs.

op m = [[1; 2]; [3; 4]; [5; 6]].

lemma test : col m 0 0 = [1;3;5].
proof.
done.
qed.

lemma test2 : row m 0 = [1;2].
proof.
done.
qed.

op transpose z (xs : 'a list list) = map (fun j => col xs j z) (iota_ 0 (size (row xs 0))).

lemma test3 : transpose 0 m = [[1;3;5]; [2;4;6]].
proof.
rewrite /transpose /m.
simplify.
rewrite /row.
...
lemma size_transpose (xs : 'a list list) z n :
  xs <> [] =>
  all (fun x => size x = n) xs =>
  size (transpose z xs) = n.
proof.
elim xs.
simplify.
done.
...
lemma size_transpose_col (xs : 'a list list) z n :
  all (fun x => size x = n) xs =>
  size (transpose z xs) = size (row xs 0).
proof.
elim xs.
simplify.
smt.
...
lemma nth_row (xs : 'a list list) i :
  nth [] xs i = row xs i.
proof.
smt.
qed.
(*
lemma row_transpose z (xs : 'a list list) i :
  row (transpose z xs) i = col xs i z.
proof.
elim xs.
smt.
progress.
rewrite /row /transpose /col /row.
progress.
have ->: map (fun (x0 : 'a list) => nth z x0 i) l = col l i z. rewrite /col. done.
...
lemma nth_transpose z (xs : 'a list list) i j n :
  xs <> [] =>
  all (fun x => size x = n) xs =>
  nth z (nth [] (transpose z xs) i) j = nth z (nth [] xs j) i.
proof.
rewrite nth_row.
...
op transpose_rec z acc (xs : 'a list list) =
  with xs = [] => []
  with xs = x :: xs' => 
    if x = [] then rev acc
    else transpose_rec z (map (head z) xs) (map behead xs).
