...
theory InTheHead5P.
  ...
  clone import InTheHead as InTheHead5P with 
    theory SS <- SS5, type witness_t = witness_t, type instance_t = instance_t.
  ...
  clone import SecretSharingSchemeSecurity with theory SecretSharingScheme <- SS5, type aux_t = statement_t.
  clone import ZeroKnowledgeSigmaProtocol with theory SigmaProtocol <- InTheHead.
  ...
  section ZeroKnowledge.
    ...
    module RealEvaluator0 (MV : MVerifier_t) = {
      ...
      proc eval(w : witness_t, x : statement_t, rp : prover_rand_t) : (bool * InTheHead5P.trace_t) option = {
        var r, c, ch, resp;
        var r_ss, r_mpc, rcs;
        var ws, x_mpc, tr, y;
        ...
        x_mpc <- map (fun pid => (pid, (x,oget (assoc ws pid)))) pid_set;
      ...
      proc guess(fc : CS.commitment_t) : bool = {
        ...
        if (valid_rand rcsl /\ valid_rand rcsm) {
          ...
          if (!bad) {
            ...
            acceptance <- InTheHead5P.check (x,c,ch) resp;
            r <- (acceptance, ((c, ch, resp)));
            b <@ D.guess(w,x,fst r, snd r);
          } else { b <@ D.guess(w,x,witness<:bool * ZKPProtocol.trace_t>.`1, witness<:bool * ZKPProtocol.trace_t>.`2); }
        }
        else { b <${0,1}; }
    ...
    module F (D : Distinguisher_t) (RSS : SecretSharingSchemeSecurity.Rand_t,
      ...
      var inst : instance_t
      var i, j, k, l, m : pid_t
      var circ : Circuit.circuit_t
      var xs : inputs_t
      var cr : pid_t list

      proc choose(circ_ : Circuit.circuit_t, w : env_input_t, aux : Privacy.aux_t) : inputs_t * pid_t list = {
        var rss, ws;
        ...
        if (valid_rand rss) {
          ws <- share rss w;
          xs <- map (fun pid => (pid, (inst, oget (assoc ws pid)))) pid_set;
          cr <- [i;j];
        }
      ...
      proc guess(vsc : views_t) : bool = {
        ...
        if (valid_rand rcsi /\ valid_rand rcsj /\ valid_rand rcsk /\
          ...
          if (!bad) {
            ...
            r <- (acceptance, ((c, ch, resp)));
            b <@ D.guess(w,inst,fst r,snd r);
          } else { b <@ D.guess(w,inst, witness<:bool * ZKPProtocol.trace_t>.`1, witness<:bool * ZKPProtocol.trace_t>.`2); }
        } else { b <$ {0,1}; }
      ...
      proc guess(s : secret_t, ss : shares_t) : bool = {
        ...
        if (valid_rand rcsi /\ valid_rand rcsj /\ valid_rand rcsk /\
          ...
          if (!bad) {
            ...
            acceptance <- InTheHead5P.check (inst, c, ch) resp;
            r <- (acceptance, ((c, ch, resp)));
            b <@ D.guess(s, inst, fst r,snd r);
          } else { b <@ D.guess(s, inst, witness<:bool * ZKPProtocol.trace_t>.`1, witness<:bool * ZKPProtocol.trace_t>.`2); }
        } else { b <$ {0,1}; }
    ...
    proof.
      ...
      call (_ : true); wp; call (_ : true); wp; skip; progress.
        ...
        have : exists tr y, (tr,y) = protocol relc rmpc (map (fun (pid : SS5.pid_t) => (pid, (x{2}, oget (assoc (share rss w{2}) pid)))) SS5.pid_set).
          by exists (protocol relc rmpc (map (fun (pid : SS5.pid_t) => (pid, (x{2}, oget (assoc (share rss w{2}) pid)))) SS5.pid_set)).`1 (protocol relc rmpc (map (fun (pid : SS5.pid_t) => (pid, (x{2}, oget (assoc (share rss w{2}) pid)))) SS5.pid_set)).`2 => /#.
        move => H0; elim H0 => tr y H0 /=; rewrite -H0 /=.
        by rewrite -H0 /= -eq_in_map /=. 
        rewrite /commitment /response /=. 
        elim (rp{2}) => rss rmpc rcs /=.
        have : exists tr y, (tr,y) = protocol relc rmpc (map (fun (pid : SS5.pid_t) => (pid, (x{2}, oget (assoc (share rss w{2}) pid)))) SS5.pid_set).
          by exists (protocol relc rmpc (map (fun (pid : SS5.pid_t) => (pid, (x{2}, oget (assoc (share rss w{2}) pid)))) SS5.pid_set)).`1 (protocol relc rmpc (map (fun (pid : SS5.pid_t) => (pid, (x{2}, oget (assoc (share rss w{2}) pid)))) SS5.pid_set)).`2 => /#.
        move => H1; elim H1 => tr y H1 /=; rewrite -H1 /=.
        rewrite /accepting_conversation /InTheHead.check; congr. split => //.
    ...
    proof.
      ...
      seq 1 1 : (#pre /\ ={ch}); first call (_ : true); skip; progress. 
        ...
        by case (B.i{2}); case (B.j{2}) => /=; 
          simplify; progress; smt(challenge_distr_diff assocP1 assocP2 assocP3 assocP4 assocP5).
        by smt().
...
by rewrite H3 /=.
      ...
      sp; seq 1 1 : (#pre /\ rmpc{1} = r{2}); first
        by call (_ : true); skip; progress; exists ([]) ([]).
      case (valid_rands c{2} r{2}); last first.
        rcondf{2} 1; first by move => &m; wp; skip => /#. 
        rcondf{1} 7; first move => &m. 
          wp; do (call (_ : true); first by if => //); wp; skip; progress.
          by rewrite /valid_rand_prover /= H3 /= /#.
        by rnd; wp; do call{1} ccsrandp_ll; wp; skip; progress.
...
have ->: map
  (fun (pid : pid_t) =>
     (pid,
      (oget
         (assoc
            (map (fun (pid0 : SS5.pid_t) => (pid0, (aux{hr}, oget (assoc (share rss{hr} x{hr}) pid0))))
               Top.InTheHead5P.pid_set) pid)).`2)) ProtocolFunctionality.pid_set = 
map
  (fun (pid : pid_t) =>
...
smt().
      ...
      seq 1 1 : (#pre /\ ={ch}).
        call (_ : true); skip; progress; last by smt().
          rewrite -eq_in_map => pid; progress; rewrite /sinput /input /view /= !map_assoc //=;
            first 2 by smt().
      ...
      sp 2 1; if => //; last first.
        wp; call (_ : true); wp; skip; progress.
        rewrite /sinput /input /pid_set /pid_mpc_set /= assocP1 assocP2 assocP3 assocP4 assocP5 /=.
        have ->: [(P1, oget (assoc (share rss{2} x{2}) P1)); (P2, oget (assoc (share rss{2} x{2}) P2)); (P3, oget (assoc (share rss{2} x{2}) P3)); (P4, oget (assoc (share rss{2} x{2}) P4)); (P5, oget (assoc (share rss{2} x{2}) P5))] = map (fun pid => (pid, oget (assoc (share rss{2} x{2}) pid))) pid_set by rewrite /pid_set /pid_mpc_set /=.
        move : (eq_assoc_map (share rss{2} x{2})).
        by rewrite unzip1_share pid_set_uniq /=; smt(SS5.correct).
      wp; call (_ : true); wp; skip; progress.
        * rewrite /sinput /input /pid_set /pid_mpc_set /= 
                  assocP1 assocP2 assocP3 assocP4 assocP5 /=.
          have ->: [(P1, oget (assoc (share rss{2} x{2}) P1)); (P2, oget (assoc (share rss{2} x{2}) P2)); (P3, oget (assoc (share rss{2} x{2}) P3)); (P4, oget (assoc (share rss{2} x{2}) P4)); (P5, oget (assoc (share rss{2} x{2}) P5))] = map (fun pid => (pid, oget (assoc (share rss{2} x{2}) pid))) pid_set by rewrite /pid_set /pid_mpc_set /=.
          move : (eq_assoc_map (share rss{2} x{2})).
          by rewrite unzip1_share pid_set_uniq /=; smt(SS5.correct).
        * congr. split; first by done.
          rewrite -eq_in_map => pid; progress => //.
    ...
    proof.
      ...
      sp; seq 1 1 : (#pre /\ rmpc{1} = r{2}); first
        by call (_ : true); skip; progress; exists ([]) ([]).
      case (valid_rands c{2} r{2}); last first.
        rcondf{2} 1; first by move => &m; skip; progress; rewrite H4 /=.
        rcondf{1} 7; first move => &m. 
          wp; do (call (_ : true); first by if => //); wp; skip; progress.
          by rewrite /valid_rand_prover /= H3 /= /#.
        by rnd; wp; do call{1} ccsrandp_ll; wp; skip; progress.
...
smt().
      ...
      sp; if{1}; last first. 
        ...
        move : H5; rewrite /valid_rand_prover /= H3 /=; move : H4.
        elim relc => topo gg /=.
        elim topo => np ns m q => -> /=.
        rewrite /tuple5_to_list /assemble /=.
        move : H H0 H2; case (F.i{hr}); case (F.j{hr}) => //=; smt(@List challenge_distr_diff).
    ...
    proof.
      ...
      sp; if{1}; last first.
        rcondf{2} 5; first move => &m; wp; call (_ : true); skip; progress.
          move : H4; rewrite /valid_rand_prover /= H3.
          elim relc => topo gg.
          elim topo => np ns m q /=.
...
case (exists (ss : (pid_t * share_t) list),
  s{2} = reconstruct ss /\
  let xs = map (fun (pid : pid_t) => (pid, (aux{2}, oget (assoc ss pid)))) SS5.pid_set in
  let outs = f relc xs in all (fun (x : pid_t * bool) => x.`2) outs); last first.
rewrite existsN.
progress.
move : (H5 (share r{2} s{2})).
...
progress.
move : (H8 (share r{2} s{2}) (share r{2} s{2})).
have <-: s{2} = reconstruct (share r{2} s{2}).
smt.
simplify.
progress.
rewrite /output /=.
have : x2 \in f relc (map (fun (pid : pid_t) => (pid, (aux{2}, oget (assoc (share r{2} s{2}) pid)))) SS5.pid_set).
smt.
have : !x2.`2 by smt().
progress.
move : H10; rewrite allP.
progress.
move : (H10 (G.i{2}, oget (assoc (f relc (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} s{2}) pid)))) SS5.pid_set)) G.i{2}))).
have ->: (G.i{2},
  oget
    (assoc (f relc (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} s{2}) pid)))) SS5.pid_set))
       G.i{2})) \in
 f relc (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} s{2}) pid)))) SS5.pid_set).
...
rewrite allP /output.
progress.
move : (H5 (G.i{2}, oget (assoc (f relc
        (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} (reconstruct ss2)) pid))))
           Top.InTheHead5P.pid_set)) G.i{2}))).
...
have ->: (G.i{2},
  oget
    (assoc
       (f relc
          (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} (reconstruct ss2)) pid))))
             Top.InTheHead5P.pid_set)) G.i{2})) \in
 f relc (map (fun (pid : pid_t) => (pid, (aux{2}, oget (assoc ss2 pid)))) SS5.pid_set).
move : (good_circuit aux{2} (reconstruct ss2)).
progress.
move : (H7 (share r{2} (reconstruct ss2)) ss2).
have ->: reconstruct (share r{2} (reconstruct ss2)) = reconstruct ss2.
smt.
simplify.
progress.
rewrite H8.
...
move : H10; rewrite allP.
progress.
move : (H10 (G.j{2}, oget (assoc (f relc (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} s{2}) pid)))) SS5.pid_set)) G.j{2}))).
have ->: (G.j{2},
  oget
    (assoc (f relc (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} s{2}) pid)))) SS5.pid_set))
       G.j{2})) \in
...
rewrite allP /output.
progress.
move : (H5 (G.j{2}, oget (assoc (f relc
        (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} (reconstruct ss2)) pid))))
           Top.InTheHead5P.pid_set)) G.j{2}))).
...
have ->: (G.j{2},
  oget
    (assoc
       (f relc
          (map (fun (pid : SS5.pid_t) => (pid, (aux{2}, oget (assoc (share r{2} (reconstruct ss2)) pid))))
             Top.InTheHead5P.pid_set)) G.j{2})) \in
 f relc (map (fun (pid : pid_t) => (pid, (aux{2}, oget (assoc ss2 pid)))) SS5.pid_set).
move : (good_circuit aux{2} (reconstruct ss2)).
progress.
move : (H7 (share r{2} (reconstruct ss2)) ss2).
...
smt(challenge_mem_pid_set).
      rcondt{2} 4; first by move => &m; wp; skip; progress;
        (move : H4; rewrite /valid_rand_prover /=;
        elim (relc) => topo gg /=;
        elim topo => np ns m q /=;
        by move : H H0 H2; case (G.i{hr}); case (G.j{hr}) => //=; smt(challenge_distr_diff)).
      sp; seq 1 1 : (#pre /\ ={ch}).
...
(*    lemma gen_response_independent (rp w inst circ c ch : 
      InTheHead5P.response rp (w, (circ, inst)) c ch =
      InTheHead5P.response witness (witness, (circ, inst)) c ch by done.*)
    ...
    proof.
      ...
      if{2}; last first.
        rcondf{1} 9; first move => &m.
          wp; do (call (_ : true); first by if => //); call (_ : true); wp; skip; progress.
          by rewrite /valid_rand_prover /= H4 /= /#.
        by rnd; wp; do call{1} ccsrandp_ll; wp; call{1} rmpc_ll; wp; skip.
      ...
      sp; if{1}; last first.
        rcondf{2} 9; first move => &m; wp; call (_ : true); wp; call (_ : true); skip; progress.
          move : H5; rewrite /valid_rand_prover /= H4.
          elim relc => topo gg.
          elim topo => np ns m q /=.
          have ->: unzip1 (tuple5_to_list (assemble (G.i{hr}, rcsi{hr}) (G.j{hr}, rcsj{hr}) (RP.k{m}, rcsk{hr}) (RP.l{m}, rcsl{hr}) (RP.m{m}, rcsm{hr}))) = SS5.pid_set.
            rewrite /valid_rands /= /tuple5_to_list /= /assemble /=.
      ...
      wp; call (_ : true); wp; skip; progress.
        * rewrite /accepting_conversation /InTheHead.check. congr. congr.
          * rewrite /response /= /get_party_committed_view /= /view /input /rand /trace /=.
          rewrite -eq_in_map => pid; progress.
          by rewrite /sinput /input /view /= !map_assoc //=; first by smt().
