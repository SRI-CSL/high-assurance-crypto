...
theory RAMProver.
  ...
  clone import Language with
    type L = circuit_t,
    type public_t = LPZKEfficient.instance_t,
    type secret_t = LPZKEfficient.witness_t * LPZKEfficient.prover_rand_t,
    type input_t = secret_t * public_t,
    type output_t = LPZKEfficient.commitment_t.
  ...
  op step (st : state_t) : state_t * execution_info_t option = 
    if has_input st then
      ...
      let zc = LPZKEfficient.gen_z (oget st.`r) c'.`gates (oget st.`inst) (oget st.`w) in
  ...
  op get_output (st : state_t) : output_t option = 
    if st.`zo <> None then
      let z = oget st.`zo in
      let z' = (LPZKEfficient.get_a (oget st.`r) (oget st.`circ).`gates) in
      Some (z, z')
    else None.
