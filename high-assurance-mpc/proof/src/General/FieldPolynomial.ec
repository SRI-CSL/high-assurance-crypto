...
op update (p : polynomial) (e : int) (x : t) : polynomial =
  with p = [] => []
  with p = m :: ms => 
    if m.`expo = e then {| coef = x ; expo = e |} :: ms
    else m :: update ms e x.

(* -------------------------------------------------------------------- *)
lemma update_id (p : polynomial) (e : int) (x : t) :
  update (update p e x) e x = (update p e x).
proof. by elim p; progress; case (x0.`expo = e) => H0 //=; rewrite H0 /=. qed.
...
proof. 
  ...
  progress; case ({| coef = x; expo = e; |} = x1) => H2; first by smt().
