...
module BGWAdditionData = struct
  ...
  let party_exec (r : rand_t) (x : input_t * input_t) : output_t = let (x1,x2) = x in fadd x1 x2
  ...
  let local_party_exec (x : input_t * input_t) (r : rand_t) (tr : trace_t) : output_t = let (x1,x2) = x in fadd x1 x2
...
module BGWAdditionGate = AdditionGate (BGWAdditionData)
